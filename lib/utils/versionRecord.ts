...
export async function insertVersionRecord(db: any, input: VersionRecordInput): Promise<void> {
  ...
  if (input.loaders.length > 0) {
    stmts.push(db.insert(versionLoaders).values(input.loaders.map((loader) => ({ versionId: input.id, loader }))));
  }
  if (input.mcVersions.length > 0) {
    stmts.push(db.insert(versionMcVersions).values(input.mcVersions.map((mcVersion) => ({ versionId: input.id, mcVersion }))));
  }
