...
describe("compactMcVersions", () => {
  ...
  it("同一プレフィックス of マイナーバージョンが2つ以上連続している場合は ～ で範囲表示されること", () => {
    expect(compactMcVersions(["1.20.2", "1.20.1"])).toEqual(["1.20.2～1.20.1"]);
  });

  /**
   * 1.21.2, 1.21.1, 1.21 -> 1.21.2～1.21 (要素数3、かつ1.21.0(1.21)まであるが、3要素のため範囲表示)
   * 1.20.4, 1.20.3, 1.20.2, 1.20.1, 1.20 -> 1.20.x (5つ連続して0で終わるので、全体が 1.20.x)
   */
  it("混在している場合に正しくソート・集約が行われること", () => {
    const input = ["1.21.2", "1.21.1", "1.21", "1.20.4", "1.20.3", "1.20.2", "1.20.1", "1.20"];
    expect(compactMcVersions(input)).toEqual(["1.21.2～1.21", "1.20.x"]);
  });
});
