...
export async function authenticateBearer(db: Database, request: Request): Promise<BearerAuth | null> {
...
export async function requireBearerScope(db: Database, request: Request, scope: string): Promise<RequireResult> {
  const token = await authenticateBearer(db, request);
