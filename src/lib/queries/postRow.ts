...
function dropChildId<T extends { id: string }>(child: T): Omit<T, "id"> {
  const { id: _id, ...rest } = child;
  return rest;
}

export function toProjectPost(row: { posts: Post; projects: ProjectFields }): ProjectPost {
  return {
    ...row.posts,
    kind: "project",
...
export function toIdeaPost(row: { posts: Post; ideas: IdeaFields }): IdeaPost {
  return {
    ...row.posts,
    kind: "idea",
 ...
 */
export function toPostAuthor(row: {
  id: string;
