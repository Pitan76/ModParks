...
export async function POST(request: Request) {
  ...
  if (grantType === "authorization_code") return handleAuthorizationCode(db, client, form);
  if (grantType === "refresh_token") return handleRefreshToken(db, client, form);

  return tokenError("unsupported_grant_type", `Unsupported grant_type: ${grantType ?? "(missing)"}`);
}

async function handleAuthorizationCode(db: Database, client: OAuthClient, form: URLSearchParams) {
  ...
  const issued = await issueTokens(db, { clientId: client.id, userId: record.userId, scopes });

  return tokenResponse(db, issued, { userId: record.userId, clientId: client.id, scopes, nonce: record.nonce });
}

async function handleRefreshToken(db: Database, client: OAuthClient, form: URLSearchParams) {
  const refreshToken = form.get("refresh_token");
  if (!refreshToken) return tokenError("invalid_request", "refresh_token is required");

  const issued = await rotateRefreshToken(db, refreshToken, client.id);
  if (!issued) return tokenError("invalid_grant", "Refresh token is invalid, expired, or already used");

  return tokenResponse(db, issued, null);
}
 ...
 */
async function tokenResponse(db: Database, issued: IssuedTokens, idTokenCtx: IdTokenContext | null) {
  ...
  if (idTokenCtx?.scopes.includes("openid")) {
    const claims = await buildUserClaims(db, idTokenCtx.userId, idTokenCtx.scopes);
    if (claims) {
      try {
