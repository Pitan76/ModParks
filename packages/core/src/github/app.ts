 ...
 *
 * App の ID と秘密鍵は秘密値なので、呼び出し側が環境から取って渡す（core は環境へ触らない）:
...
/** 両方揃っているときだけ App を使える */
export function githubAppConfig(appId: string | undefined, privateKey: string | undefined): GithubAppConfig | null {
...
async function createAppJwt({ appId, privateKey }: GithubAppConfig): Promise<string> {
 ...
 */
export async function findInstallationIdForRepo(app: GithubAppConfig, repoFullName: string): Promise<number | null> {
  const jwt = await createAppJwt(app);
