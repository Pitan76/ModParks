...
import { fetchCfGameVersionMap, resolveCfGameVersionIds, updateCfFileGameVersions } from "@modparks/core/curseforgeUpload";
...
/**
 * 選択した複数バージョンへ、対応MCバージョンを一括で追加する。
 * Next の Server Action と modparks-api の両方から呼ぶ。キャッシュの無効化は呼び出し側で行う。
 *
...
export async function batchAddMcVersion(
  { db, t }: { db: Database; t: ServerErrorTranslator },
  userId: string,
  projectSlug: string,
  versionIds: string[],
  mcVersions: string[],
